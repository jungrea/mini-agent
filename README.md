...
- **消息结构安检**：每次 LLM 调用前走 `normalize_messages`——重排 user 块、补缺失 `tool_result`、剥内部字段、兜底空 content，把"源头瑕疵"降级为日志，不再直接 400
...
3. **执行** — 并行桶走 `ThreadPoolExecutor(max_workers=PARALLEL_MAX_WORKERS=3)`；串行桶照旧带 spinner 逐个跑
